...
def check_eligibility(resume_string, job_description_files, threshold=0.1):
    ...
    for job_file in job_description_files:
        ...
        if similarity_score > threshold:
            eligible_jobs[job_file] = similarity_score
    return eligible_jobs

def main():
    st.set_page_config(
        page_title="SkillForge - AI Gatekeeper",
        page_icon="🤖",
        layout="wide",
    ...
    new_title = '<p style="font-family:KaTeX_Caligraphic; color:#d2dacd; font-size: 45px; text-align: center; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5);"><strong>SkillForge AI Gatekeeper</strong></p>'
    ...
    if success and eligible_jobs:
        ...
        # Sort eligible jobs by similarity score
        sorted_eligible_jobs = sorted(eligible_jobs.items(), key=lambda x: x[1], reverse=True)

        for job_file, similarity_score in sorted_eligible_jobs:
            job_name = os.path.basename(job_file).split(".")[0]
            st.markdown(f'<span style="color: black; font-size: 18px;"><em><strong>~  {job_name}</em></strong> (Similarity Score: {similarity_score:.2f})</span>', unsafe_allow_html=True)
    elif success:
